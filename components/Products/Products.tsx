...
import { pricies } from '../../utils/prices.json';
...
const Products = () => {
    ...
    const [sortOption, setSortOption] = useState<SORTOPTION>(SORTOPTION.PRICE); //price,alphabet
    ...
    const applyAllFilters = () => {
        let catFilterResult = applyCategoryFilter(categoryFilter, products);
        ...
        <div className={styles.products}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                <div className="hidden md:block">
                    <div className="py-5">
                        <FilterList filterName={'category'} filterListTitle={'Category'} multi={true} cb={updateCategoryFilter} filters={categories} values={categoryFilter} />
                    </div>
                    <hr className="mr-4" />
                    <div className="py-5">
                        <FilterList filterName={'price'} filterListTitle={FILTER_LIST_TITLE.PRICE} multi={false} cb={updatePriceFilter} filters={pricies} values={[priceFilter]} />
                    </div>
                ...
                <div className="col-span-1 lg:col-span-3 md:col-span-1 justify-self-center md:justify-self-end">
                    <ProductList products={currentProducts.slice(pagesVisited, (pagesVisited + productsPerPage))} updateCartItems={updateCartItems} />
...
const ProductSectionHeader = ({ sortByPrice, sortAlphabetically, sortOption, sortOrder, setSortOrder, updateCategoryFilter, updatePriceFilter, showMobileFilter, setShowMobileFilter, categoryFilter, priceFilter, resetFilter }: any) => {
    ...
    return (
        <div className={styles['products__header']}>
            <div className={`${styles['products__header-title']} flex flex-wrap`}>
                <span className={styles['products__header-title--left']}>Photography / </span><span className={`${styles['products__header-title--right']} pl-2`}>Premium Photos</span>
            </div>
            <div className="md:flex hidden">
                ...
                <div className="flex">
                    ...
                    <select onChange={handleSortSelect} className={styles['products__header-select']} value={sortOption}>
            ...
            {showMobileFilter && <div className={`${styles.mobile__filter} block md:hidden`}>
                <div className="flex justify-between align-center">
                    ...
                    <img src="./icons/close.svg" width={18} height={18} onClick={() => { setShowMobileFilter(false) }}/>
                </div>
                <div className={styles['mobile__filter-body']}>
                    <div className="py-5">
                        <FilterList filterName={'category_mobile'} filterListTitle={FILTER_LIST_TITLE.CAREGORY} multi={true} cb={updateCategoryFilter} filters={categories} values={categoryFilter} />
                    </div>
                    <hr className="mr-4" />
                    <div className="py-5">
                    <FilterList filterName={'price_mobile'} filterListTitle={FILTER_LIST_TITLE.PRICE} multi={false} cb={updatePriceFilter} filters={pricies} values={[priceFilter]} />
                    </div>
                </div>
